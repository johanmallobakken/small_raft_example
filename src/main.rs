...
use raft::{prelude::*, StateRole, RaftLog};
use regex::Regex;
...
fn main() {
    ...
    let blocked = Arc::new(Mutex::new(HashSet::<u64>::new()));
    let log_lengths = Arc::new(Mutex::new(Vec::<u64>::from([0, 0, 0])));
    ...
    for (i, rx) in rx_vec.into_iter().enumerate() {
        ...
        let log_lengths = Arc::clone(&log_lengths);
        ...
        let handle = thread::spawn(move || loop {
            ...
            loop {
                // Report log lengths
                match &node.raft_group {
                    Some(raw_node) => {
                        let idx: usize = (raw_node.raft.id-1).try_into().unwrap();
                        let mut log_lengths = log_lengths.lock().unwrap();
                        log_lengths[idx] = raw_node.raft.raft_log.committed
                    },
                    None => {},
                };
                ...
                match node.my_mailbox.try_recv() {
                    Ok(msg) => {
                        ...
                        if (&node.raft_group).is_some(){
                            let raw_node = node.raft_group.as_ref().unwrap();
                            println!("message from: {} to: {}", msg.from, &raw_node.raft.id);
                            if blocked.contains(&msg.from) && blocked.contains(&raw_node.raft.id) {
                                println!("not sent");
                                break;
                            }
                        }
            ...
            println!("!!!!!!!!!! ROLE OF NODE IS: {:?}, ID IS: {}, TERM: {}", raft_group.raft.state ,raft_group.raft.id, raft_group.raft.term);
    ...
    (0..100u16)
        .filter(|i| {
            if !done{
                done = partial_connectivity(&log_lengths, &blocked);
                for _ in 0..20 {
                    println!("BROKEN LINK!!!!!!!!!!!!!!!! {}", done)
                }
            }
...
fn partial_connectivity(log_lengths: &Arc<Mutex<Vec<u64>>>, broken: &Arc<Mutex<HashSet<u64>>>) -> bool{
    let mut log_lengths = log_lengths.lock().unwrap();

    if log_lengths.iter().min().unwrap() < &5 {
        return false
    }
    ...
    println!("lens {:?}", log_lengths);

    if log_lengths[0] >= log_lengths[1] && log_lengths[1] > log_lengths[2]{
        broken.insert(1);
        broken.insert(3);
        println!("broke 1, 3");
        return true; 
    ...
    if log_lengths[0] >= log_lengths[2] && log_lengths[2] > log_lengths[1]{
        broken.insert(1);
        broken.insert(2);
        println!("broke 1, 2");
...
impl Node {
    ...
    fn block_node(&mut self, node_id: u64){
        self.blocked = node_id;
    }


    fn get_raft_log(&self) -> Option<&RaftLog<MemStorage>>{
        match &self.raft_group {
            Some(raw_node) => {
                Some(&raw_node.raft.raft_log)
            },
            None => None,
        }
    }
